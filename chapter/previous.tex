...
\section{语言模式}
本模板内含三套语言环境 \lstinline{lang=cn}、\lstinline{lang=en} 以及 \lstinline{lang=it} \footnote{由 \href{https://github.com/VincentMVV}{VincentMVV} 提供意大利语翻译，具体的内容见\href{https://github.com/ElegantLaTeX/etbook/issues/85}{Italian translation}。}，改变语言环境会改变图表标题的引导词（图，表），文章结构词（比如目录，参考文献等），以及定理环境中的引导词（比如定理，引理等）。不同语言模式的启用如下：
\begin{lstlisting}
\documentclass[cn]{etbook} 
\documentclass[lang=cn]{etbook}
\end{lstlisting}
...
\section{设备选项}
最早我们在 ElegantNote 模板中加入了设备选项（\lstinline{device}），后来，我们觉得这个设备选项的设置可以应用到 etbook 中\footnote{不过因为 etbook 模板封面图片的存在，在修改页面设计时，需要对图片进行裁剪。}，而且 Book 一般内容比较多，如果在 iPad 上看无需切边，放大，那用户的阅读体验将会得到巨大提升。你可以使用下面的选项将版面设置为 iPad 设备模式\footnote{默认为 normal 模式，也即 A4 纸张大小。}
\begin{lstlisting}
\documentclass[pad]{etbook} %or
\documentclass[device=pad]{etbook}
\end{lstlisting}
...
\begin{lstlisting}
\documentclass[green]{etbook} %or
\documentclass[color=green]{etbook}
\end{lstlisting}

\begin{table}[htbp]
  \caption{etbook 模板中的颜色主题\label{tab:color thm}}
  \centering
...
\begin{lstlisting}
\documentclass[hang]{etbook} %or
\documentclass[titlestyle=hang]{etbook}
\end{lstlisting}

在章标题内，章节编号默认是以数字显示，也即{\kaishu 第 1 章}，{\kaishu 第 2 章}等等，如果想要把数字改为中文，可以使用
\begin{lstlisting}
\documentclass[chinese]{etbook} %or
\documentclass[scheme=chinese]{etbook}
\end{lstlisting}
...
\begin{lstlisting}
\documentclass[simple]{etbook} %or
\documentclass[mode=simple]{etbook}
\end{lstlisting}
...
\begin{itemize}
  \item \textit{定理类环境}，包含标题和内容两部分，全部定理类环境的编号均以章节编号。根据格式的不同分为 3 种
    \begin{itemize}
      ...
      \item \textcolor{second}{\textbf{theorem、lemma、corollary、proposition}} 环境，颜色为 \textcolor{second} {second}；
      \item \textcolor{third}{\textbf{封面色条}} 环境，颜色为 \textcolor{third}{third}。
    \end{itemize}
...
\begin{lstlisting}
\documentclass[cite=super]{etbook} % set super style ref style
\documentclass[super]{etbook}
\documentclass[cite=authoryear]{etbook} % set author year ref style
\documentclass[authoryear]{etbook}
\end{lstlisting}
...
\begin{lstlisting}
  \documentclass[bibstyle=apalike]{etbook} 
\end{lstlisting}
...
\begin{lstlisting}
\documentclass[twocol]{etbook}
\documentclass[toc=twocol]{etbook}
\end{lstlisting}
...
\begin{lstlisting}
\documentclass[math=newtx]{etbook}
\end{lstlisting}
...
请在 \lstinline{etbook.cls} 内搜索 \lstinline{yhmath} 或者 \lstinline{esvect}，将你所需要的宏包加载语句\textit{取消注释}即可。
\begin{lstlisting}
...
\begin{enumerate}[itemsep=1.5ex]
  ...
  \item \question{我该使用什么版本？}
    请务必使用\href{https://github.com/ElegantLaTeX/etbook/releases}{最新正式发行版}，发行版间不定期可能会有更新（修复 bug 或者改进之类），如果你在使用过程中没有遇到问题，不需要每次更新\href{https://github.com/ElegantLaTeX/etbook/archive/master.zip}{最新版}，但是在发行版更新之后，请尽可能使用最新版（发行版）！最新发行版可以在 GitHub 或者 \TeX{} Live 2019 内获取。
  ...
  \item \question{您好，我们想用您的 etbook 模板写一本书。关于机器学习的教材，希望获得您的授权，谢谢您的宝贵时间。}
  ...
  \item \question{代码高亮环境能用其他语言吗？}
    可以的，etbook 模板用的是 \lstinline{listings} 宏包，你可以在环境（\lstinline{lstlisting}）之后加上语言（比如 Python 使用 \lstinline{language=Python} 选项），全局语言修改请使用 \lstinline{lsset} 命令，更多信息请参考宏包文档。
...
根据用户的反馈，我们不断修正和完善模板。截止到此次更新，etbook 模板在 GitHub 上有将近 100 次提交，正式发行版本（release）有 16 次。由于 3.00 之前版本与现在版本差异非常大，在此不列出 3.00 之前的更新内容。
